    ...
    try {
      ...
      setError(
        err.response?.data?.message ||
          "Animal records are temporarily unavailable. Please try again shortly."
      );
    } finally {
      setLoading(false);
    }
      ...
      <div className="flex-1 space-y-4">
        ...
        {error ? (
          <div className="panel space-y-4">
            <SectionHeader
              ...
              title="Animal records are temporarily unavailable"
              description={error}
            />
          </div>
        ) : null}
        ...
        {user?.role !== "user" ? (
          <div className="glass rounded-2xl p-4 space-y-3">
            <p className="text-sm font-semibold">Add animal</p>
            <form className="grid gap-2 md:grid-cols-4" onSubmit={createAnimal}>
              <input
        ...
        <div className="space-y-3">
          {!loading &&
            !error &&
            animals.map((a) => (
              <div key={a._id} className="glass rounded-2xl p-4 space-y-2">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-slate-500">
                      Breed: {a.breed || "N/A"} | Status: {a.status}
                    </p>
                    {a.foodPreferences?.length > 0 ? (
                      <p className="text-[11px] text-slate-500">
                        Food: {a.foodPreferences.join(", ")}
                      </p>
                    ) : null}
                  </div>
                  {user?.role !== "user" ? (
                  ...
                  <div className="space-y-2">
                    <div className="grid gap-2 md:grid-cols-3">
                      <input
                        ...
                        value={health.note}
                        onChange={(e) => setHealth({ ...health, note: e.target.value })}
                      />
                      <input
                        className="rounded-xl border px-3 py-2"
                        placeholder="Doctor"
                      ...
                      />
                      <button className="btn-secondary" onClick={() => addHealth(a._id)}>
                        Save note
                      </button>
                    </div>
                    ...
                    <div className="grid gap-2 md:grid-cols-4">
                      <input
                        className="rounded-xl border px-3 py-2"
                        type="date"
                        value={appt.scheduledFor}
                        onChange={(e) => setAppt({ ...appt, scheduledFor: e.target.value })}
                      />
                      <input
                        className="rounded-xl border px-3 py-2"
                        placeholder="Doctor"
                      ...
                      <input
                        ...
                        value={appt.recommendation}
                        onChange={(e) => setAppt({ ...appt, recommendation: e.target.value })}
                      />
                      <button className="btn-secondary md:col-span-4" onClick={() => addAppt(a._id)}>
                        Save appointment
                      </button>
                    </div>
                  </div>
                  ...
                  {a.healthUpdates?.slice(0, 3).map((h, idx) => (
                    <div key={idx} className="rounded-lg bg-white/50 px-2 py-1">
                      ...
                      <div className="text-[11px] text-slate-500">
                        {h.doctorName || "Doctor"} | {new Date(h.checkupDate).toLocaleDateString()}
                      </div>
                    </div>
                  ))}
                  {a.doctorAppointments?.slice(0, 2).map((d, idx) => (
                    <div key={idx} className="rounded-lg bg-white/40 px-2 py-1">
                      <div className="font-medium">
                        Appointment: {new Date(d.scheduledFor).toLocaleDateString()}
                      </div>
                      <div className="text-[11px] text-slate-500">
                        {d.doctorName || "Doctor"} | {d.recommendation || "Checkup"}
                      </div>
                    </div>
                  ))}
                </div>
